...
{
    public class F74_1 : FormulaBase, IFormula
    {
        public Output Calculate(Input input)
        {
            ...
            if (input.Tinggi > 24)
            {
                result.HargaKainB = Math.Round((input.Lebar * 3.5) / 39 * input.HargaKainA * input.Set, 2);
                result.HargaKainC = Math.Round((input.Lebar * 3.5) / 39 * input.HargaKainA * input.Set, 2);
            }
            else
            {
